...
sed -i 's/-ffor-scope//g' src/Makefile.am
./autogen.sh || error "Autogen failed"
./configure CXXFLAGS="-std=c++11" || error "configure failed"
...
# Desktop, Docs and Icon (FIX: Se corrigió 'packman' por 'pacman')
cp pacman/pacman_sdl.desktop "$PACKAGE_NAME/usr/share/applications/"
cp pacman/{README,COPYING,AUTHORS,ChangeLog,INSTALL,NEWS,TODO} "$PACKAGE_NAME/usr/share/doc/pacman_sdl/"
